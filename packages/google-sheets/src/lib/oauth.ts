...
export class OAuthSession {
  ...
  constructor(options: OAuthSessionOptions) {
    ...
    this.scopes = [SHEETS_SCOPE, DRIVE_METADATA_READ_SCOPE];
  }
  ...
  private async authorizeInteractive(): Promise<StoredToken> {
    ...
    const authPromise = new Promise<string>((resolve, reject) => {
      ...
      server.on("request", (req, res) => {
        ...
        res.setHeader("content-type", "text/html; charset=utf-8");
        res.end(
          "<html><body><h1>Google Sheets MCP authorized</h1><p>You can close this tab.</p></body></html>"
        );
        resolve(code);
      });
    });
...
export async function loadOAuthCredentialsFromEnvironment(): Promise<OAuthCredentials> {
  const clientId = process.env.MCP_GOOGLE_SHEETS_CLIENT_ID?.trim();
  const clientSecret = process.env.MCP_GOOGLE_SHEETS_CLIENT_SECRET?.trim();

  if (clientId && clientSecret) {
    return { clientId, clientSecret };
  }

  const credentialsPath = process.env.MCP_GOOGLE_SHEETS_CLIENT_CREDENTIALS_PATH?.trim();
  if (!credentialsPath) {
    throw new ToolExecutionError(
      "Missing Google OAuth credentials. Set MCP_GOOGLE_SHEETS_CLIENT_ID and MCP_GOOGLE_SHEETS_CLIENT_SECRET, or MCP_GOOGLE_SHEETS_CLIENT_CREDENTIALS_PATH.",
      { kind: "config_error" }
    );
  }
...
export function isKeytarEnabled(): boolean {
  return parseBooleanEnv(process.env.MCP_GOOGLE_SHEETS_USE_KEYTAR);
}
